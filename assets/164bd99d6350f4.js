const cv8=strawberry.create("app"); const sy9=cv8.factory,ih8=cv8.service,cs8=cv8.component; sy9('xa2',function(){return{root:'https://www.kryptonite-apis.cloud',tenantId:'80e28231798c479493ef6e252bca216f',variantId:'32ec4fd0a38f4b8b8bf2a24f4658e3a2'}});ih8('kr5',(xa2)=>{return{create:{stage:(emailAddress)=>{return new Promise(function(resolve,reject){fetch(xa2.root+'/customer/anonymous',{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:xa2.tenantId,email_address:emailAddress})}).then((response)=>response.json()).then((data)=>{resolve(data.customer.id)}).catch(reject)})}}}});sy9('xa2',function(){return{root:'https://www.kryptonite-apis.cloud',tenantId:'80e28231798c479493ef6e252bca216f',variantId:'32ec4fd0a38f4b8b8bf2a24f4658e3a2'}});ih8('ya4',(xa2)=>{return{create:({emailAddress,taskDescription,todoItems,orderId})=>{return new Promise(function(resolve,reject){fetch(xa2.root+'/task/create',{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({order_id:orderId,task_description:taskDescription,todo_items:todoItems})}).then((response)=>response.json()).then((task)=>{resolve(task)}).catch(reject)})},get:{external_id:(internalTaskId)=>{return new Promise((resolve,reject)=>{fetch(xa2.root+'/tasks/get/external_id?task_id='+internalTaskId,{method:"GET",headers:{"Content-Type":"application/json"}}).then((response)=>response.json()).then((task)=>{resolve(task.external_id)}).catch(reject)})},internalAndEmail:(customerEmail,taskId)=>{return new Promise((resolve,reject)=>{fetch(xa2.root+'/public/get/task?task_id='+taskId+'&customer_email='+customerEmail,{method:"GET",headers:{"Content-Type":"application/json"}}).then((response)=>response.json()).then((task)=>{if(task.hasOwnProperty('message')){reject(new Error(task.message))
return}
resolve(task)}).catch(reject)})}},pay:({orderId,externalId})=>{return new Promise(function(resolve,reject){fetch(xa2.root+'/task/settle/unpaid',{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order_id:orderId,external_id:externalId})}).then((response)=>response.json()).then((task)=>{resolve(task)}).catch(reject)})}}});ih8('hd7',(xa2)=>{return{create:({taskId,todos,token})=>{return new Promise(function(resolve,reject){fetch(xa2.root+'/todos/create',{method:"POST",headers:{"Content-Type":"application/json","X-Krypton-Token":token},body:JSON.stringify({task_id:taskId,todos:todos})}).then((response)=>response.json()).then((todoData)=>{resolve({})}).catch(reject)})}}});ih8('gj6',()=>{function between(min,max){return Math.floor(Math.random()*(max-min+1)+min)}
...
$scope.isMenuOpen=true}}});cs8('ke4',($scope,$patch)=>{$scope.say_hello='Hello World!';});cs8('ya3',($scope,$patch)=>{$scope.say_hello='Hello World!';});cs8('gl1',($scope,$patch)=>{$scope.say_hello='Hello World!';});cs8('tb4',($scope,$patch,gj6,$hide,$show,xa2,ya4,hd7,kr5)=>{$scope.todos=[]
$scope.onStep=1
...
$scope.todos.push({id:gj6.generator.id(),description:todoDescription})})
if(addTodoDescription!==undefined&&addTodoDescription!==null){$scope.todos.push({id:gj6.generator.id(),description:addTodoDescription})
return}
patchComponent()}
...
try{$scope.customerId=await kr5.create.stage($scope.contactEmail)
...
return fetch(xa2.root+"/paypal/order/create",{method:"POST",headers:{"Content-Type":"application/json",},body:JSON.stringify({tenant_id:xa2.tenantId,customer_id:$scope.customerId,cart:[{variant_id:xa2.variantId,quantity:1}],currency_code:'USD',discount_code:'NODISCOUNT'})}).then(function(res){return res.json();}).then(function(data){document.querySelector('#payment_status_indicator').innerText=''
